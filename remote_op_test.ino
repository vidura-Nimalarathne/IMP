...
#define TURN_TIME   200    // NEW: for A/D turning
...
void loop() {
  ...
  if (isMoving) {
    ...
    if ((currentMode == 'F' && elapsed >= FWD_TIME) ||
        ...
        (currentMode == 'L' && elapsed >= TURN_TIME) ||   // NEW
        (currentMode == 'D' && elapsed >= TURN_TIME)) {   // NEW
...
void processPacket(String pkt) {
  ...
  switch (cmd) {
    ...
    case 'C':
      rotateInPlace();   // your old rotate (kept)
      motionStartTime = millis();
      currentMode = 'C';
      isMoving = true;
      sendACK("C");
...
// Rotate in place (your old rotate)
void rotateInPlace() {
...
// NEW: turn counter-clockwise (left turn)
// typical: left wheel reverse, right wheel forward
void turnCCW() {
  motor1.setSpeed(-SPEED_TURN);
  motor2.setSpeed(-SPEED_TURN);  // NOTE: because your motor2 sign is inverted in forward()
  Serial.println("Turn CCW");
}

// NEW: turn clockwise (right turn)
// typical: left wheel forward, right wheel reverse
void turnCW() {
  motor1.setSpeed(+SPEED_TURN);
  motor2.setSpeed(+SPEED_TURN);  // NOTE: adjust if direction is wrong
